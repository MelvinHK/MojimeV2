...
function NavigateBtn({ type }: NavigateBtnProps) {
  const { anime, currentIndex, setCurrentIndex } = useContext(AnimeContext);

  const hasPrevious = useMemo(() => currentIndex > 0, [currentIndex]);
  const hasNext = useMemo(() => anime && currentIndex < anime.episodes.length - 1, [currentIndex]);
  ...
  return (
    <button className="media-button" onClick={navigate} disabled={type === "next" ? !hasNext : !hasPrevious}>
      {type === "next" ?
